...
class TheseusClient:
    ...
    async def create_letter(
        ...
        logger.info(
            f"Creating letter in queue {queue_name} for {address['first_name']}, {address['country']} - stamps: {rubber_stamps}"
        )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code not in (200, 201):
                    logger.error(
                        f"Theseus API error creating letter: status {response.status_code}"
                    )
    ...
    async def get_letter_status(self, letter_id: str) -> dict[str, Any]:
        """
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"Theseus API error getting letter status: status {response.status_code}"
                    )
                    raise TheseusAPIError(
                        f"Theseus API error: status {response.status_code}",
                        status_code=response.status_code,
    ...
    async def mark_letter_mailed(self, letter_id: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(url, headers=self._get_headers(), timeout=30.0)
                ...
                if response.status_code != 200:
                    logger.error(
                        f"Theseus API error marking letter mailed: status {response.status_code}"
                    )
                    raise TheseusAPIError(
                        f"Theseus API error: status {response.status_code}",
                        status_code=response.status_code,
    ...
    if notify_slack:
        try:
            ...
            await slack_bot.send_error_notification(
                event_name=event_name, error_message=error_message, request_summary=request_summary
            )
