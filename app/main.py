...
async def create_letter(
    ...
    if not letter_id:
        raise HTTPException(
            status_code=502,
            detail="Mail service returned an invalid letter ID. Please try again later.",
        )
    ...
    if request.mail_type == MailType.PARCEL:
        if request.weight_grams is None:
            raise HTTPException(status_code=400, detail="Weight is required for parcels.")
            ...
            if action_id.startswith("mark_mailed:"):
                ...
                if trigger_id and order:
                    await slack_bot.open_update_tracking_modal(
                        trigger_id, order_id, order.tracking_code
                    )
