...
async def lifespan(app: FastAPI):
    ...
    await slack_bot.send_server_lifecycle_notification(
        "startup", f"API v{app.version} running on {settings.api_host}:{settings.api_port}"
    )
...
async def create_letter(
    ...
    try:
        if (
            request.mail_type == MailType.BUBBLE_PACKET
            and request.weight_grams
            and request.weight_grams > 500
        ):
            raise HTTPException(
                status_code=400,
        ...
        assert request.weight_grams is not None
        cost_cents = calculate_cost(
            mail_type=request.mail_type, country=request.country, weight_grams=request.weight_grams
        )
    ...
    try:
        theseus_response = await theseus_client.create_letter(
            queue_name=event.theseus_queue,  # type: ignore[arg-type]
            address=address,
        ...
        await slack_bot.send_error_notification(
            event_name=event.name,  # type: ignore[arg-type]
            error_message=str(e),
    ...
    try:
        ...
        letter.slack_message_ts = message_ts  # type: ignore[assignment]
        letter.slack_channel_id = channel_id  # type: ignore[assignment]
    ...
    if request.mail_type == MailType.PARCEL:
        await slack_bot.send_parcel_quote_request(
            event_name=event.name,  # type: ignore[arg-type]
            weight_grams=request.weight_grams,  # type: ignore[arg-type]
            country=request.country,
            recipient_name=f"{request.first_name} {request.last_name}",
            rubber_stamps_raw=request.rubber_stamps,
            letter_id=letter_id,  # type: ignore[arg-type]
    ...
    try:
        ...
        full_address += (
            f"<br>{request.city}, {request.state}, {request.postal_code}<br>{request.country}"
        )
        ...
        await airtable_client.create_record(
            ...
            email_reason="Letter",
            record_id=letter_id,  # type: ignore[arg-type]
            ysws=event.name,  # type: ignore[arg-type]
    ...
    return LetterResponse(
        ...
        status=LetterStatus.QUEUED,
        theseus_url=theseus_client.get_public_letter_url(letter_id),  # type: ignore[arg-type]
        email_sent=True,
    )
...
async def mark_event_paid(
    event_id: int, _: bool = Depends(verify_admin_api_key), db: AsyncSession = Depends(get_db)
):
    ...
    previous_balance: int = event.balance_due_cents  # type: ignore[assignment]
    event.balance_due_cents = 0  # type: ignore[assignment]
    event.is_paid = True  # type: ignore[assignment]
...
async def get_financial_summary(
    _: bool = Depends(verify_admin_api_key), db: AsyncSession = Depends(get_db)
):
    ...
    for event in events:
        ...
        unpaid_events.append(
            UnpaidEvent(
                event_id=event.id,  # type: ignore[arg-type]
                event_name=event.name,  # type: ignore[arg-type]
                balance_due_usd=cents_to_usd(event.balance_due_cents),  # type: ignore[arg-type]
                letter_count=event.letter_count,  # type: ignore[arg-type]
                stamps=StampCounts(canada=ca_count, us=us_count, international=int_count),
                last_letter_at=last_letter_at,
            )
        )
        total_due_cents += int(event.balance_due_cents)
        total_ca += ca_count
        total_us += us_count
        total_int += int_count
...
async def calculate_shipping_cost(request: Request, body: CostCalculatorRequest):
    ...
    try:
        ...
        cost_cents = calculate_cost(
            mail_type=body.mail_type, country=body.country, weight_grams=body.weight_grams
        )
...
def get_404_html(
    title: str = "Page Not Found", message: str = "The page you're looking for doesn't exist."
) -> str:
...
async def create_order(
    ...
    try:
        ...
        order.slack_message_ts = message_ts  # type: ignore[assignment]
        order.slack_channel_id = channel_id  # type: ignore[assignment]
    except Exception as e:
...
async def get_order_status_page(
    ...
    if not order:
        return HTMLResponse(
            content=get_404_html(
                "Order Not Found", "This order does not exist or the link is invalid."
            ),
            status_code=404,
        )
    ...
    if order.status == OrderStatus.PENDING:
        ...
        if order.fulfillment_note:
            escaped_note = html.escape(str(order.fulfillment_note))
            fulfillment_info = f'<p class="note">{escaped_note}</p>'
        if order.tracking_code:
            escaped_tracking = html.escape(str(order.tracking_code))
...
async def get_order_status_api(request: Request, order_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return OrderStatusResponse(
        ...
        tracking_code=order.tracking_code,  # type: ignore[arg-type]
        fulfillment_note=order.fulfillment_note,  # type: ignore[arg-type]
        created_at=order.created_at,  # type: ignore[arg-type]
        fulfilled_at=order.fulfilled_at,  # type: ignore[arg-type]
    )
...
async def update_financial_canvas(db: AsyncSession):
    ...
    for event in events:
        ...
        total_due_cents += event.balance_due_cents  # type: ignore[assignment]
        total_letters += event.letter_count  # type: ignore[assignment]
        total_ca += ca_count
        total_us += us_count
        total_int += int_count
...
async def handle_slack_interactions(
    ...
    if payload_type == "view_submission":
        ...
        if callback_id.startswith("fulfill_order_modal:"):
            ...
            tracking_code = (
                values.get("tracking_code_block", {}).get("tracking_code", {}).get("value")
            )
            fulfillment_note = (
                values.get("fulfillment_note_block", {}).get("fulfillment_note", {}).get("value")
            )
            ...
            if order:
                order.status = OrderStatus.FULFILLED  # type: ignore[assignment]
                order.fulfilled_at = datetime.utcnow()  # type: ignore[assignment]
                ...
                if event and order.slack_message_ts and order.slack_channel_id:
                    await slack_bot.update_order_fulfilled(
                        channel_id=order.slack_channel_id,  # type: ignore[arg-type]
                        message_ts=order.slack_message_ts,  # type: ignore[arg-type]
                        event_name=event.name,  # type: ignore[arg-type]
                        order_id=order.order_id,  # type: ignore[arg-type]
                        order_text=order.order_text,  # type: ignore[arg-type]
                        status_url=get_order_status_url(order.order_id),  # type: ignore[arg-type]
                        tracking_code=order.tracking_code,  # type: ignore[arg-type]
                        fulfillment_note=order.fulfillment_note,  # type: ignore[arg-type]
                        fulfilled_at=order.fulfilled_at,  # type: ignore[arg-type]
                    )
            ...
            if not tracking_code or len(tracking_code) < 1:
                return JSONResponse(
                    content={
                        "response_action": "errors",
                        "errors": {"tracking_code_block": "Tracking code is required"},
                    }
                )
            if len(tracking_code) > 64:
                return JSONResponse(
                    content={
                        "response_action": "errors",
                        "errors": {
                            "tracking_code_block": "Tracking code must be 64 characters or less"
                        },
                    }
                )
    ...
    if payload_type == "block_actions":
        ...
        for action in actions:
            ...
            if action_id.startswith("mark_mailed:"):
                ...
                if letter:
                    try:
                        await theseus_client.mark_letter_mailed(letter.letter_id)  # type: ignore[arg-type]
                    except TheseusAPIError as e:
                        logger.error(
                            f"Failed to mark letter {letter.letter_id} as mailed in Theseus: {e}"
                        )

                    letter.status = LetterStatus.SHIPPED  # type: ignore[assignment]
                    letter.mailed_at = datetime.utcnow()  # type: ignore[assignment]
                    ...
                    if event and letter.slack_message_ts and letter.slack_channel_id:
                        await slack_bot.update_letter_shipped(
                            channel_id=letter.slack_channel_id,  # type: ignore[arg-type]
                            message_ts=letter.slack_message_ts,  # type: ignore[arg-type]
                            event_name=event.name,  # type: ignore[arg-type]
                            queue_name=event.theseus_queue,  # type: ignore[arg-type]
                            recipient_name=f"{letter.first_name} {letter.last_name}",
                            country=letter.country,  # type: ignore[arg-type]
                            rubber_stamps_raw=letter.rubber_stamps_raw,  # type: ignore[arg-type]
                            cost_cents=letter.cost_cents,  # type: ignore[arg-type]
                            letter_id=letter.letter_id,  # type: ignore[arg-type]
                            mailed_at=letter.mailed_at,  # type: ignore[arg-type]
                        )
                ...
                if trigger_id and order:
                    await slack_bot.open_update_tracking_modal(
                        trigger_id, order_id, order.tracking_code  # type: ignore[arg-type]
                    )
...
async def catch_all_404(path: str, request: Request):
    ...
    return JSONResponse(content={"detail": "Not Found", "path": f"/{path}"}, status_code=404)
