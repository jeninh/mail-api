...
async def handle_update_tracking_modal(ack, _body, view):
    ...
    if not tracking_code or len(tracking_code) < 1:
        await ack(
            response_action="errors", errors={"tracking_code_block": "Tracking code is required"}
        )
        return
...
async def handle_paid_command(client, trigger_id, respond):
    ...
    if not events:
        await respond(response_type="ephemeral", text="✅ No unpaid events found!")
        return
    ...
    for event in events:
        ...
        options.append(
            {
                "text": {"type": "plain_text", "text": f"{event.name} - ${balance_usd:.2f}"},
                "value": str(event.id),
            }
        )
    ...
    modal = {
        ...
        "title": {"type": "plain_text", "text": "Mark Event as Paid"},
        "submit": {"type": "plain_text", "text": "Confirm Payment"},
        "close": {"type": "plain_text", "text": "Cancel"},
        "blocks": [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": "Select an event to mark as paid:"},
            },
            {
                "type": "input",
                ...
                "element": {
                    ...
                    "placeholder": {"type": "plain_text", "text": "Choose an event"},
                    "options": options,
                },
    ...
    try:
        ...
        await respond(response_type="ephemeral", text="❌ Failed to open modal. Please try again.")
...
async def handle_financial_command(respond):
    ...
    async with get_async_session_local()() as db:
        ...
        if not events:
            await respond(
                response_type="ephemeral", text="✅ No unpaid events! All balances are settled."
            )
            return
